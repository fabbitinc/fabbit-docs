...
이 문서는 `.wireframe` 앱과 기획 작성 체계에 관한 작업 전달 메모다. 현재 제품
정책, 사업 정책, 확정 기획의 기준 본문이 아니라 구현 전달과 운영 정리에 쓰는
운영용 handoff 문서로 읽어야 한다.

## 문서 등급과 사용 범위

이 문서는 `.wireframe` 앱 구축 과정과 기획 문서 열람 체계를 정리한다. 내용이
현재 정책 SSOT나 상위 기획 문서와 충돌하면 이 문서가 아니라 해당 기준 문서를
우선한다.

- 정책 기준은 `10_제품/정책/*`, `40_사업/정책/*`를 먼저 따른다.
- 제품 구조와 기획 본문은 `10_제품/기획/*`를 먼저 따른다.
- 이 문서는 구현 handoff와 운영 메모를 보존하는 용도로 유지한다.
